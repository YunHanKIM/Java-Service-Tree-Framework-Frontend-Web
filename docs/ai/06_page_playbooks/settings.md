...
두 개의 독립된 폼(각자 별도 `<form onSubmit>`, 별도 저장 버튼) — 하나의 폼으로 합치지 말 것,
...
2. **AI 연동**: `GET/POST /api/settings/ai` — 서버 호출이라 실패할 수 있다(토스트로 에러 표시).
   ...
   기존 값으로 채워줄 방법이 없다. placeholder로 `maskedKey`만 보여준다.
...
- `apiKey`를 빈 문자열로 제출하면 서버(`saveAiSettings`)는 **기존 키를 유지**한다(지우려는 게 아니라
  ...
  provider를 바꾸면서 키를 비워두면 "이전 provider의 키가 새 provider에 남는" 것이 아니라
  `current.provider === input.provider` 체크로 인해 `null`이 된다(`settings-store.ts` 참조) — provider를
  바꿀 때는 항상 새 키를 같이 입력해야 한다는 뜻이므로 UI에서 이 제약을 안내하는 문구 추가를 고려할 것.
