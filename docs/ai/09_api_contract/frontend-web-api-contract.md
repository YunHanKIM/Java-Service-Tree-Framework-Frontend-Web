...
| `/api/settings/ai` | GET/POST | **로그인한 계정 전용** AI 프로바이더·API 키 조회/저장(`userId`로 격리). GET 응답은 `{provider, hasKey, maskedKey, model}` — **원문 키는 절대 응답에 포함하지 않는다** |
...
`ai/*` 라우트는 로그인한 계정에 등록된 API 키가 없으면 **412**(`code: "NO_API_KEY"`)를 반환한다.
클라이언트는 이 코드를 보고 설정 페이지로 안내하는 알림을 띄운다(mock으로 대체하지 않음 — 실패를
정직하게 보여준다).

## AI 프로바이더 — 실제 외부 API 호출 (계정별 키, 전역 아님)
...
직접 발급받아 입력하며(ChatGPT Plus·Claude Pro 구독과는 별개, 사용량 과금), **로그인한 계정에만
연결되어** `.data/ai-settings.json`에 `userId`별로 저장되고 브라우저로 재전송되지 않는다 — 한 계정의
키가 다른 계정(데모 계정 포함)의 AI 호출에 쓰이는 일은 없다(`12_known_issues` 참조, 공개 배포 시
비용 격리를 위한 핵심 안전장치). 응답은 zod 스키마(`src/lib/ai/schemas.ts`)로 검증하고, 마크다운
