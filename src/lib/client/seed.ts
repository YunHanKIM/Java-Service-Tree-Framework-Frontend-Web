...
import { applicationsStore, generateId, postingsStore, resumeStore } from "./store";
...
export function ensureSeedData(): void {
  if (postingsStore.list().length > 0) return;
  ...
  postingDefs.forEach((def) => {
    const posting = postingsStore.create({
      source: "paste",
      company: def.company,
      title: def.title,
    ...
    const application = applicationsStore.create({ postingId: posting.id, stage: def.stage });
    if (def.resultNote) {
      applicationsStore.updateResultNote(application.id, def.resultNote);
    }
  });

  const first = postingsStore.list().find((p) => p.company === "ABC테크");
  if (first) {
    postingsStore.saveAnalysis(first.id, {
      matchingSkills: [
      ...
      missingSkills: [{ name: "React Query", reason: '공고에서 발췌: "React Query, 상태 관리 라이브러리 사용 경험이 있으면 좋습니다."' }],
      prepItems: [
        { id: generateId("prep"), label: "React Query의 기본 개념과 주요 사용 패턴 복습하기", done: false },
        { id: generateId("prep"), label: "최근 진행한 프로젝트를 STAR 기법으로 정리하기", done: false },
        { id: generateId("prep"), label: "ABC테크의 서비스와 기술 블로그를 살펴보고 관심 있는 주제 정리하기", done: false },
      ],
    });
  }
}
